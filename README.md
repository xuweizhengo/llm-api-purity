...
You can import the public proxyai.best directory as the initial site catalog:

```bash
npm run import:proxyai
```

The importer reads `https://api.proxyai.best/api/v1/public/proxyai/sites`, converts the public fields into `data/sites.json`, and keeps source metadata on each imported site.
...
可以导入 proxyai.best 的公开目录作为初始站点库：

```bash
npm run import:proxyai
```

导入脚本读取 `https://api.proxyai.best/api/v1/public/proxyai/sites`，把公开字段转换成 `data/sites.json`，并在每个导入站点上保留来源信息。
